function data = loadData_box(dataPath, subjectID)
...
if ~isfolder(subjectPath)
    error('Subject folder not found for subject %s in %s', subjectID, dataPath);
end

dayFolders = dir(fullfile(subjectPath, [subjectID '_20*']));
dayFolders = dayFolders([dayFolders.isdir]);  % ignore zip/files automatically

if isempty(dayFolders)
    error('No day folders found for subject %s in %s', subjectID, subjectPath);
end
